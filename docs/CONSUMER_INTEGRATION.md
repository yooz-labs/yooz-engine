...
| Generation | `POST /v1/infinite/sessions/:id/generate`; generates on Swift-runtime-supported models (Qwen3.6 `qwen3_5_moe`, native window ≤262K). Gemma4 and retrieval return `501 generation_unavailable` until the Swift port lands (#184) |
...
| Infinite `generate` returns 501 | The active model has no Swift MLX backend yet (Gemma4 / retrieval — #184); session state is preserved | Switch to a Swift-runtime-supported model (Qwen3.6 `qwen3_5_moe`), or branch on the stable `generation_unavailable` code. Create/append/checkpoint/delete keep working regardless. |
