...
| Consumer that COPIES the prebuilt `dist/YoozEngineXPC.xpc` artifact (rather than building the `xpc-service` target from source per this recipe — this is whisper's actual pattern, `scripts/embed-engine-xpc.sh`) sees the service fail library validation or lose capabilities after re-signing | A shallow `codesign --force --sign <identity>` on the copied `.xpc` re-signs only the OUTER bundle: the nested frameworks stay signed with THIS repo's own identity (a Team ID mismatch under hardened-runtime library validation, since they were never re-signed with the consumer's own identity), and re-signing without `--entitlements` drops app-sandbox/network.client/application-groups even if the outer signature itself still verifies (see the C1 finding on engine PR #256 for why a valid signature alone doesn't prove entitlements survived) | Deep-re-sign every nested framework under `Contents/Frameworks` with your OWN identity, THEN re-sign the `.xpc` itself with `--entitlements` pointing at your OWN resolved entitlements file — mirroring `scripts/resign-embedded-xpc.sh` in this repo, not just a single flat `codesign --force`. Tracked consumer-side as whisper#276. |
