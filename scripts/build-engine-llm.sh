#!/usr/bin/env bash
#
# build-engine-llm.sh — one-command build + sign for the YoozEngineLLM
# variant (LLM generation/classification only, no speech stack).
#
# Produces:   dist/YoozEngineLLM.app   (== `Yooz Engine (LLM).app`)
# Scheme:     YoozEngineLLM            (defined in project.yml)
...
#
# LLM-only variant (engine#297) targeted at remi-class consumers that want
# generation/classification without any speech stack. AppleSTTModule,
# GrammarModule, STTModule, VADModule, and InfiniteModule are not linked —
# the first variant to drop AppleSTT + Grammar on top of Lite's existing
# MLX-STT + VAD exclusions.
...
SCHEME="YoozEngineLLM"
CONFIG="${YOOZ_ENGINE_CONFIG:-Debug}"
APP_NAME="Yooz Engine (LLM).app"
BIN_NAME="Yooz Engine (LLM)"
DIST_DIR="$ROOT/dist"
DIST_APP="$DIST_DIR/YoozEngineLLM.app"
DERIVED_DATA="$ROOT/.build/DerivedData-EngineLLM"
BUILD_LOG="$ROOT/.build/engine-llm-build.log"
IDENTITY="${YOOZ_SIGNING_IDENTITY:-}"
# Prefer an LLM-specific entitlements file if one ever lands; fall back to
# the full engine's entitlements (current state per project.yml). Mirrors
# the pattern in build-engine-lite.sh / build-whisper-helper.sh:147-149.
ENTITLEMENTS="$ROOT/YoozEngine/YoozEngineLLM.entitlements"
[[ -f "$ENTITLEMENTS" ]] || ENTITLEMENTS="$ROOT/YoozEngine/YoozEngine.entitlements"

log() { printf "[build-engine-llm] %s\n" "$*"; }
fail() { printf "[build-engine-llm] ERROR: %s\n" "$*" >&2; exit 1; }
